...
const clinicSchema = yup.object({
  ...
  stow_username: yup.string().when('is_stow_enabled', {
    is: true,
    then: (schema) => schema.required('Username is required'),
    otherwise: (schema) => schema.notRequired(),
  }),
  stow_password: yup.string().when('is_stow_enabled', {
    is: true,
    then: (schema) => schema.required('Password is required'),
    otherwise: (schema) => schema.notRequired(),
  }),
  stow_get: yup.string().when('is_stow_enabled', {
    is: true,
    then: (schema) => schema.required('GET (QIDO) is required'),
    otherwise: (schema) => schema.notRequired(),
  }),
  stow_post: yup.string().when('is_stow_enabled', {
    is: true,
    then: (schema) => schema.required('POST (STOW) is required'),
    otherwise: (schema) => schema.notRequired(),
  }),
});
...
const buildDefaults = (data) => ({
  ...
  stow_password: data?.stow_password || '',
  stow_get: data?.stow_get || '/dicom-rs/qido/studies',
  stow_post: data?.stow_post || '/dicom-rs/stow/studies',
});
...
const ClinicForm = ({ clinic, onClose }) => {
  ...
  const isStowEnabled = watch('is_stow_enabled');
  ...
  const onSubmit = async (data) => {
    ...
    formData.append('stow_password', data.stow_password || '');
    formData.append('stow_get', data.stow_get || '');
    formData.append('stow_post', data.stow_post || '');
        ...
        <div className="bg-white px-5 p-4">
          ...
          <div className='mt-3'>
            ...
            /> */}
              ...
              <div className="border-t border-gray-200 pt-4">
                <FormField
                  label="STOW-RS Enabled"
                  name="is_stow_enabled"
                  type="checkbox"
                  registration={register('is_stow_enabled')}
                />
                <em className='text-gray-500 text-xs'>(Enabling STOW-RS allows you to upload DICOM studies to the server.)</em>
              </div>
                ...
                <div className="space-y-4 mt-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField
                      label="Username"
                      name="stow_username"
                      registration={register('stow_username')}
                      placeholder="Enter STOW Username"
                      required
                      error={errors.stow_username?.message}
                    />
                    <FormField
                      label="Password"
                      name="stow_password"
                      type="password"
                      registration={register('stow_password')}
                      placeholder="••••••••••••"
                      required
                      error={errors.stow_password?.message}
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField
                      label="GET (QIDO)"
                      name="stow_get"
                      value="/dicom-rs/qido/studies"
                      registration={register('stow_get')}
                      required
                      readOnly={true}
                      toggleableReadOnly={true}
                      error={errors.stow_get?.message}
                    />
                    <FormField
                      label="POST (STOW)"
                      name="stow_post"
                      value="/dicom-rs/stow/studies"
                      registration={register('stow_post')}
                      required
                      readOnly={true}
                      toggleableReadOnly={true}
                      error={errors.stow_post?.message}
                    />
                  </div>
                </div>
              )}
